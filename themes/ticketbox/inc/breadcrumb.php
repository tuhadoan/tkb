<?php
if(!class_exists('DTBreadcrumb')):
class DTBreadcrumb {
	public function __construct(){
		if(!is_admin()){
			add_filter('dt_get_breadcrumb', array(&$this,'output'));
		}
	}
	...
	public function output($args = array()){
		...
		$defaults = array(
				'home' => esc_html_e( 'Home', 'ticketbox' ),
				'delimiter' => '',
				'front_page' => false,
				'show_blog' => true,
		...
		$args = apply_filters( 'dt_breadcrumb_args', $args );
		$args = wp_parse_args( $args, $defaults );
		
		$delimiter = $args['delimiter'];
		
		if ( is_front_page() && !$args['front_page'] )
			return apply_filters( 'dt_breadcrumb_output', false );
		...
		/* If viewing any type of archive. */
			...
			if ( is_category() || is_tag() || is_tax() ) {
				...
				if(is_numeric(get_query_var('w') && 0 !== get_query_var('w') ))
					$item['last'] =  sprintf( esc_html_e('Weekly Archive for: "%s"','ticketbox'),get_the_time('W'));
				elseif ( is_day() )
				$item['last'] = sprintf( esc_html_e('Daily Archive for: "%s"','ticketbox'),get_the_time('F jS, Y'));
				elseif ( is_month() )
				$item['last'] =  sprintf( esc_html_e('Monthly Archive for: "%s"','ticketbox'),get_the_time('F jS, Y'));
				elseif ( is_year() )
				$item['last'] =  sprintf(esc_html_e('Yearly Archive for: "%s"','ticketbox'),get_the_time('Y'));
			}
		
			else if ( is_author() )
				$item['last'] =  sprintf(esc_html_e('Author Archive for: "%s"','ticketbox'),get_the_author_meta( 'display_name', $wp_query->post->post_author ));
		}
		
		/* If viewing search results. */
		else if ( is_search() )
			$item['last'] =  esc_html_e('Search','ticketbox');//sprintf(esc_html_e('search Results for: "%s"','ticketbox'),stripslashes( strip_tags( get_search_query() ) ));
			
		/* If viewing a 404 error page. */
		else if ( is_404() )
			$item['last'] = esc_html_e( 'Page Not Found', 'ticketbox' );
		...
		$breadcrumb = apply_filters( 'dt_breadcrumb_output', $breadcrumb );
	...
	protected function _bbpress_items() {
		...
		elseif ( bbp_is_single_topic() ) {
			...
			if ( bbp_is_topic_split() )
				$item[] = esc_html_e( 'Split', 'ticketbox' );
	
			elseif ( bbp_is_topic_merge() )
			$item[] = esc_html_e( 'Merge', 'ticketbox' );
	
			elseif ( bbp_is_topic_edit() )
			$item[] = esc_html_e( 'Edit', 'ticketbox' );
		}

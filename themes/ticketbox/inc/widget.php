...
class DT_Social_Widget extends DT_Widget {
	public function __construct(){
		...
		$this->widget_name        = esc_html__( 'Social', 'ticketbox' );
...
class DT_Tweets extends WP_Widget {
	public function __construct() {
		parent::__construct (
			'dt_tweets', 		// Base ID
			'Recent Tweets', 		// Name
			array ('classname'=>'tweets-widget','description' => __ ( 'Display recent tweets', 'ticketbox' ) )
		);
	}
	...
	public function widget($args, $instance) {
		...
		if(empty($instance['consumerkey']) || empty($instance['consumersecret']) || empty($instance['accesstoken']) || empty($instance['accesstokensecret']) || empty($instance['cachetime']) || empty($instance['username'])){
			echo '<strong>'.esc_html__('Please fill all widget settings!' , 'ticketbox').'</strong>' . $after_widget;
			return;
		}
		...
		if($diff >= $crt || empty($dt_widget_recent_tweets_cache_time)){
			...
			if(!require_once(DTINC_DIR . '/lib/twitteroauth.php')){
				echo '<strong>'.esc_html__('Couldn\'t find twitteroauth.php!','ticketbox').'</strong>' . $after_widget;
				return;
			}
			...
			if(!empty($tweets->errors)){
				if($tweets->errors[0]->message == 'Invalid or expired token'){
					echo '<strong>'.$tweets->errors[0]->message.'!</strong><br/>'.esc_html__('You\'ll need to regenerate it <a href="https://dev.twitter.com/apps" target="_blank">here</a>!', 'ticketbox' ) . $after_widget;
				}else{
	...
	protected function _relative_time($a=''){
		...
		if(is_numeric($d) && $d > 0) {
			...
			//if less then minute
			if($d < $minute) return sprintf(esc_html__("%s seconds ago",'ticketbox'),floor($d));
			//if less then 2 minutes
			if($d < $minute * 2) return esc_html__("about 1 minute ago",'ticketbox');
			//if less then hour
			if($d < $hour) return sprintf(esc_html__('%s minutes ago','ticketbox'), floor($d / $minute));
			//if less then 2 hours
			if($d < $hour * 2) return esc_html__("about 1 hour ago",'ticketbox');
			//if less then day
			if($d < $day) return sprintf(esc_html__("%s hours ago", 'ticketbox'),floor($d / $hour));
			//if more then day, but less then 2 days
			if($d > $day && $d < $day * 2) return esc_html__("yesterday",'ticketbox');
			//if less then year
			if($d < $day * 365) return sprintf(esc_html__('%s days ago','ticketbox'),floor($d / $day));
			//else return more than a year
			return esc_html__("over a year ago",'ticketbox');
		}
	}
	...
	public function form($instance) {
		...
		<p>
			<label>' . __ ( 'Consumer Secret' , 'ticketbox' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Access Token' , 'ticketbox' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Access Token Secret' , 'ticketbox' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Cache Tweets in every' , 'ticketbox' ) . ':</label>
			<input type="text" name="' . $this->get_field_name ( 'cachetime' ) . '" id="' . $this->get_field_id ( 'cachetime' ) . '" value="' . esc_attr ( $instance ['cachetime'] ) . '" class="small-text" />'.esc_html__('hours','ticketbox').'
		</p>
		<p>
			<label>' . __ ( 'Twitter Username' , 'ticketbox' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Tweets to display' , 'ticketbox' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Exclude replies', 'ticketbox' ) . ':</label>
...
class DT_Post_Thumbnail_Widget extends DT_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( "Widget post with thumbnail.", 'ticketbox' );
		$this->widget_id          = 'dt_widget_post_thumbnail';
		$this->widget_name        = esc_html__( 'Post Thumbnail', 'ticketbox' );
		$this->cached = false;
		...
		$this->settings           = array(
				...
				'orderby' => array(
					...
					'options' => array(
							'latest'   => esc_html__( 'Latest', 'ticketbox' ),
							'comment'  => esc_html__( 'Most Commented', 'ticketbox' ),
					)
				),
				'categories' => array(
						'type'  => 'select',
						'std'   => '',
						'multiple'=>true,
						'label'=>esc_html__('Categories','ticketbox'),
						'desc' => esc_html__( 'Select a category or leave blank for all', 'ticketbox' ),
						'options' => $categories_options,
				),
				'hide_date' => array(
						'type'  => 'checkbox',
						'std'   => 0,
						'label' => esc_html__( 'Hide date in post meta info', 'ticketbox' )
				),
				'hide_comment' => array(
						'type'  => 'checkbox',
						'std'   => 0,
						'label' => esc_html__( 'Hide comment in post meta info', 'ticketbox' )
				),
		);
		parent::__construct();
	}
				...
				while ($r->have_posts()): $r->the_post();global $post;
					...
					echo '<div class="posts-thumbnail-content">';
						...
						if(!$hide_comment){
							...
							if ( $number > 1 ) {
								$output = str_replace( '%', number_format_i18n( $number ), ( false === false ) ? esc_html__( '% Comments' ,'ticketbox') : false );
							} elseif ( $number == 0 ) {
								$output = ( false === false ) ? esc_html__( '0 Comments', 'ticketbox') : false;
							} else { // must be one
								$output = ( false === false ) ? esc_html__( '1 Comment', 'ticketbox' ) : false;
							}
...
class DT_Mailchimp_Widget extends DT_Widget {
	public function __construct(){
		...
		$this->widget_name        = esc_html__( 'Mailchimp Subscribe', 'ticketbox' );
		$this->cached = false;
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',

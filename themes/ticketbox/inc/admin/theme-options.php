	...
	class DTThemeOptions {
		...
		protected static function getFileSystem( $url = '' ) {
			...
			if ( false === ( $creds = request_filesystem_credentials( $url, '', false, false, null ) ) ) {
				_e( 'This is required to enable file writing', 'ticketbox' );
...
<div class="wrap">
	...
	<form method="post" action="options.php" enctype="multipart/form-data">
			...
			<div class="dt-opt-heading">
				...
				<a target="_blank"
					href="http://dawnthemes.com/<?php echo basename(get_template_directory())?>/document"><?php _e('Online Document','ticketbox')?></a>
			</div>
		</div>
		<div class="clear"></div>
		<div class="dt-opt-actions">
			<em style="float: left; margin-top: 5px;"><?php echo esc_html('Theme customizations are done here. Happy styling!','ticketbox')?></em>
			<button id="dt-opt-submit" name="dt_opt_save" class="button-primary"
				type="submit"><?php echo esc_html__('Save All Change','ticketbox') ?></button>
		</div>
		<div class="clear"></div>
		...
		<div class="dt-opt-actions2">
			...
			<button id="dt-opt-reset"
				...
				type="submit"><?php echo esc_html__('Reset Options','ticketbox') ?></button>
		</div>
		<div class="clear"></div>
	</form>
</div>
...
<?php
				...
				case 'image' :
					...
					$btn_text = ! empty( $image_id ) ? esc_html__( 'Change Image', 'ticketbox' ) : esc_html__( 
						'Select Image', 
						'ticketbox' );
					...
					echo '<input type="button" class="button" id="' . $field['id'] . '_clear" value="' .
						 esc_html__( 'Clear Image', 'ticketbox' ) . '" ' .
					...
					jQuery(document).ready(function($) {
						$('#<?php echo esc_attr($field['id']); ?>_upload').on('click', function(event) {
							...
							var dt_meta_image_frame = wp.media.frames.wp_media_frame = wp.media({
								title: "<?php echo esc_html__( 'Select or Upload your Image', 'ticketbox' ); ?>",
								button: {
									text: "<?php echo esc_html__( 'Select', 'ticketbox' ); ?>"
								},
								library: { type: 'image' },
								multiple: false
							});
							...
							dt_meta_image_frame.on('select', function(){
								...
								$this.attr('value', '<?php echo esc_html__( 'Change Image', 'ticketbox' ); ?>');
						...
						$('#<?php echo esc_attr($field['id']); ?>_clear').on('click', function(event) {
							var $this = $(this);
							$this.hide();
							$('#<?php echo esc_attr($field['id']); ?>_upload').attr('value', '<?php echo esc_html__( 'Select Image', 'ticketbox' ); ?>');
...
<?php
				...
				';
					...
					echo '<select class="dt-opt-value dt-chosen-select-nostd" id="' . $field['id'] .
						 '_background_repeat" data-placeholder="' . esc_html__( 'Background Repeat', 'ticketbox' ) .
					...
					echo '<select class="dt-opt-value dt-chosen-select-nostd" id="' . $field['id'] .
						 '_background_size" data-placeholder="' . esc_html__( 'Background Size', 'ticketbox' ) . '" name="' .
					...
					echo '<select class="dt-opt-value dt-chosen-select-nostd" id="' . $field['id'] .
						 '_background_attachment" data-placeholder="' . esc_html__( 'Background Attachment', 'ticketbox' ) .
					...
					echo '<select class="dt-opt-value dt-chosen-select-nostd"  id="' . $field['id'] .
						 '_background_position" data-placeholder="' . esc_html__( 'Background Position', 'ticketbox' ) .
				...
				<script type="text/javascript">
					jQuery(document).ready(function($) {
						...
						$('#<?php echo esc_attr($field['id']); ?>_background_image_clear').on('click', function(event) {
							var $this = $(this);
							$this.hide();
							$('#<?php echo esc_attr($field['id']); ?>_background_image_upload').attr('value', '<?php echo esc_html__( 'Select Image', 'ticketbox' ); ?>');
...
<?php
				...
				case 'custom_font' :
					...
					echo '<select data-placeholder="' . esc_html__( 'Select a font family', 'ticketbox' ) .
					...
					echo '<select data-placeholder="' . esc_html__( 'Font size', 'ticketbox' ) .
						 '" class="dt-opt-value dt-chosen-select-nostd"  id="' . $field['id'] . '" name="' . $field_name .
						 '[font-size]">';
					echo '<option value=""></option>';
					...
					echo '<select data-placeholder="' . esc_html__( 'Font style', 'ticketbox' ) .
						 '" class="dt-opt-value dt-chosen-select-nostd"  id="' . $field['id'] . '" name="' . $field_name .
						 '[font-style]">';
					echo '<option value=""></option>';
					...
					echo '<select data-placeholder="' . esc_html__( 'Subset', 'ticketbox' ) .
						 '" class="dt-opt-value dt-chosen-select-nostd"  id="' . $field['id'] . '" name="' . $field_name .
						 '[subset]">';
					echo '<option value=""></option>';
				...
				case 'import' :
					...
					echo '<button id="dt-opt-import" class="button-primary" name="' . self::$_option_name .
						 '[dt_opt_import]" type="submit">' . esc_html__( 'Import', 'ticketbox' ) . '</button>';
		...
		public function get_sections() {
			$section = array( 
				'general' => array( 
					...
					'fields' => array( 
						...
						array( 
							...
							'type' => 'image', 
							'value' => get_template_directory_uri() . '/assets/images/logo.png', 
							'label' => esc_html__( 'Fixed Menu Logo', 'ticketbox' ), 
						...
						array( 
							...
							'label' => esc_html__( 'Favicon', 'ticketbox' ), 
							'desc' => esc_html__( 'Image that will be used as favicon (32px32px).', 'ticketbox' ) ), 
						array( 
							'name' => 'apple57', 
							'type' => 'image', 
				...
				'design_layout' => array( 
					...
					'desc' => __( '<p class="description">Customize Design and Layout.</p>', 'ticketbox' ), 
					'fields' => array( 
						array( 
							'name' => 'body-bg', 
							'type' => 'color', 
				...
				'color_typography' => array( 
					...
					'desc' => __( '<p class="description">Customize Color and Typography.</p>', 'ticketbox' ), 
					'fields' => array( 
						array( 
							'name' => 'brand-primary', 
							'type' => 'color', 
							'label' => esc_html__( 'Brand primary', 'ticketbox' ), 
							'value' => '#262626' ), 
						array( 
							'name' => 'brand-success', 
							'type' => 'color', 
							'label' => esc_html__( 'Brand success', 'ticketbox' ), 
							'value' => '#57bb58' ), 
						array( 
							'name' => 'brand-info', 
							'type' => 'color', 
							'label' => esc_html__( 'Brand info', 'ticketbox' ), 
							'value' => '#5788bb' ), 
						array( 
							'name' => 'brand-warning', 
							'type' => 'color', 
							'label' => esc_html__( 'Brand warning', 'ticketbox' ), 
							'value' => '#f0ad4e' ), 
						array( 
							'name' => 'brand-danger', 
							'type' => 'color', 
							'label' => esc_html__( 'Brand danger', 'ticketbox' ), 
							'value' => '#bb5857' ), 
						array( 
							'name' => 'text-color', 
							'type' => 'color', 
							'label' => esc_html__( 'Text color', 'ticketbox' ), 
							'value' => '#525252' ), 
						array( 
							'name' => 'link-color', 
							'type' => 'color', 
							'label' => esc_html__( 'Link color', 'ticketbox' ), 
							'value' => '#262626' ), 
						array( 
							'name' => 'link-hover-color', 
							'type' => 'color', 
							'label' => esc_html__( 'Link hover color', 'ticketbox' ), 
							'value' => '#262626' ), 
						array( 
							'name' => 'headings-color', 
							'type' => 'color', 
							'label' => esc_html__( 'Headings Color', 'ticketbox' ), 
							'value' => '#262626' ), 
						array( 
							'name' => 'body-typography', 
							'type' => 'custom_font', 
							'field-label' => esc_html__( 'Body', 'ticketbox' ), 
							'value' => array() ), 
						array( 
						...
						array( 
							'name' => 'h1-typography', 
							'type' => 'custom_font', 
							'field-label' => esc_html__( 'Heading H1', 'ticketbox' ), 
							'value' => array() ), 
						array( 
							'name' => 'h2-typography', 
							'type' => 'custom_font', 
							'field-label' => esc_html__( 'Heading H2', 'ticketbox' ), 
							'value' => array() ), 
						array( 
							'name' => 'h3-typography', 
							'type' => 'custom_font', 
							'field-label' => esc_html__( 'Heading H3', 'ticketbox' ), 
							'value' => array() ), 
						array( 
							'name' => 'h4-typography', 
							'type' => 'custom_font', 
							'field-label' => esc_html__( 'Heading H4', 'ticketbox' ), 
							'value' => array() ), 
						array( 
							'name' => 'h5-typography', 
							'type' => 'custom_font', 
							'field-label' => esc_html__( 'Heading H5', 'ticketbox' ), 
							'value' => array() ), 
						array( 
							'name' => 'h6-typography', 
							'type' => 'custom_font', 
							'field-label' => esc_html__( 'Heading H6', 'ticketbox' ), 
							'value' => array() ) ) ), 
				'header' => array( 
					'icon' => 'fa fa-header', 
					'title' => esc_html__( 'Header', 'ticketbox' ), 
					'desc' => __( '<p class="description">Customize Header.</p>', 'ticketbox' ), 
					'fields' => array( 
						array( 
							'name' => 'header-style', 
							'type' => 'select', 
							...
							'options' => array( 
								'center' => esc_html__( 'Center', 'ticketbox' ), 
								'below' => esc_html__( 'Below', 'ticketbox' ) ), 
							'value' => 'below' ), 
						array( 
							'name' => 'topbar_setting', 
							'type' => 'heading', 
							'text' => esc_html__( 'Topbar Settings', 'ticketbox' ) ), 
						array( 
							'name' => 'show-topbar', 
							'type' => 'switch', 
							'on' => esc_html__( 'Show', 'ticketbox' ), 
							'off' => esc_html__( 'Hide', 'ticketbox' ), 
							'label' => esc_html__( 'Display top bar', 'ticketbox' ), 
							'desc' => __( 
						...
						array( 
							...
							'label' => esc_html__( 'Left topbar content', 'ticketbox' ), 
							'options' => array( 
								'none' => esc_html__( 'None', 'ticketbox' ), 
								'menu_social' => esc_html__( 'Social', 'ticketbox' ), 
								'info' => esc_html__( 'Site Info', 'ticketbox' ), 
								'custom' => esc_html__( 'Custom HTML', 'ticketbox' ) ), 
							'value' => 'info' ), 
						array( 
							'name' => 'left-topbar-social', 
							'type' => 'muitl-select', 
							'label' => esc_html__( 'Top Social Icon', 'ticketbox' ), 
							'dependency' => array( 
						...
						array( 
							...
							'label' => esc_html__( 'Phone number', 'ticketbox' ), 
							'value' => '(123) 456 789' ), 
						array( 
							'name' => 'left-topbar-email', 
							'type' => 'text', 
							'dependency' => array( 'element' => 'left-topbar-content', 'value' => array( 'info' ) ), 
							'label' => esc_html__( 'Email', 'ticketbox' ), 
							'value' => 'info@domain.com' ), 
						array( 
							'name' => 'left-topbar-skype', 
							'type' => 'text', 
							'dependency' => array( 'element' => 'left-topbar-content', 'value' => array( 'info' ) ), 
							'label' => esc_html__( 'Skype', 'ticketbox' ), 
							'value' => 'skype.name' ), 
						array( 
							'name' => 'left-topbar-custom-content', 
							'type' => 'textarea', 
							'dependency' => array( 'element' => 'left-topbar-content', 'value' => array( 'custom' ) ), 
							'label' => esc_html__( 'Left Topbar Content Custom HTML', 'ticketbox' ) ), 
						
						array( 
							'name' => 'right-topbar-content', 
							'type' => 'buttonset', 
							'dependency' => array( 'element' => 'show-topbar', 'value' => array( '1' ) ), 
							'label' => esc_html__( 'Right topbar content', 'ticketbox' ), 
							'options' => array( 
								'none' => esc_html__( 'None', 'ticketbox' ), 
								'menu' => esc_html__( 'Navigation', 'ticketbox' ), 
								'menu_social' => esc_html__( 'Social', 'ticketbox' ), 
								'custom' => esc_html__( 'Custom HTML', 'ticketbox' ) ), 
							'value' => 'menu' ), 
						array( 
							...
							'type' => 'switch', 
							'label' => esc_html__( 'Use Account Url', 'ticketbox' ), 
							'dependency' => array( 'element' => 'right-topbar-content', 'value' => array( 'menu' ) ), 
							'desc' => esc_html__( 'Use account url in right topbar menu', 'ticketbox' ), 
							'value' => '0' ),  // 1 = checked | 0 = unchecked
						array( 
							'name' => 'topbar-social', 
							'type' => 'muitl-select', 
						...
						array( 
							...
							'label' => esc_html__( 'Right Topbar Content Custom HTML', 'ticketbox' ) ), 
						
						array( 
							'name' => 'main_navbar_setting', 
							'type' => 'heading', 
							'text' => esc_html__( 'Main Navbar Settings', 'ticketbox' ) ), 
						array( 
							'name' => 'sticky-menu', 
							'type' => 'switch', 
						...
						array( 
							...
							'type' => 'switch', 
							'label' => esc_html__( 'Custom Sticky Color', 'ticketbox' ), 
							'dependency' => array( 'element' => 'sticky-menu', 'value' => array( '1' ) ), 
							'desc' => esc_html__( 'Custom sticky menu color scheme ?', 'ticketbox' ), 
							'value' => '0' ),  // 1 = checked | 0 = unchecked
						array( 
							'name' => 'sticky-menu-bg', 
							'type' => 'color', 
							'label' => esc_html__( 'Sticky menu background', 'ticketbox' ), 
							'dependency' => array( 'element' => 'custom-sticky-color', 'value' => array( '1' ) ), 
							'value' => '' ), 
						array( 
							'name' => 'sticky-menu-color', 
							'type' => 'color', 
							'label' => esc_html__( 'Sticky menu color', 'ticketbox' ), 
							'dependency' => array( 'element' => 'custom-sticky-color', 'value' => array( '1' ) ), 
							'value' => '' ), 
						array( 
							'name' => 'sticky-menu-hover-color', 
							'type' => 'color', 
							'label' => esc_html__( 'Sticky menu hover color', 'ticketbox' ), 
							'dependency' => array( 'element' => 'custom-sticky-color', 'value' => array( '1' ) ), 
							'value' => '' ), 
						array( 
							'name' => 'menu-transparent', 
							'type' => 'switch', 
							'label' => esc_html__( 'Transparent Main Menu', 'ticketbox' ), 
							'desc' => esc_html__( 'Enable or disable main menu background transparency', 'ticketbox' ), 
							'value' => '0' ),  // 1 = checked | 0 = unchecked
						array( 
							'name' => 'ajaxsearch', 
							'type' => 'switch', 
							'label' => esc_html__( 'Ajax Search in menu', 'ticketbox' ), 
							'desc' => esc_html__( 'Enable or disable ajax search in menu.', 'ticketbox' ), 
							'value' => '1' ),  // 1
							                                                                                                     // = checked
							                                                                                                     // | 0 =
							                                                                                                     // unchecked
						array( 
							'name' => 'heading-bg', 
							'type' => 'image', 
							'desc' => esc_html__( 'Change Heading background', 'ticketbox' ), 
							'label' => esc_html__( 'Heading background', 'ticketbox' ) ), 
						array( 
							'name' => 'breadcrumb', 
							'type' => 'switch', 
							'on' => esc_html__( 'Show', 'ticketbox' ), 
						...
						array( 
							...
							'type' => 'heading', 
							'text' => esc_html__( 'Header Color Scheme', 'ticketbox' ) ), 
						array( 
							'name' => 'header-color', 
							'type' => 'select', 
							'label' => esc_html__( 'Header Color Scheme', 'ticketbox' ), 
							'desc' => esc_html__( 'Custom Topbar and Main menu color scheme .', 'ticketbox' ), 
							'options' => array( 
								'default' => esc_html__( 'Default', 'ticketbox' ), 
								'custom' => esc_html__( 'Custom', 'ticketbox' ) ), 
							'value' => 'default' ), 
						array( 
							...
							'options' => array( 
								'topbar-bg' => esc_html__( 'Topbar Background', 'ticketbox' ), 
								'topbar-font' => esc_html__( 'Topbar Color', 'ticketbox' ), 
								'topbar-link' => esc_html__( 'Topbar Link Color', 'ticketbox' ), 
								'header-bg' => esc_html__( 'Header Background', 'ticketbox' ), 
								'header-color' => esc_html__( 'Header Color', 'ticketbox' ), 
								'header-hover-color' => esc_html__( 'Header Hover Color', 'ticketbox' ), 
								'navbar-bg' => esc_html__( 'Navbar Background', 'ticketbox' ), 
								'navbar-font' => esc_html__( 'Navbar Color', 'ticketbox' ), 
								'navbar-font-hover' => esc_html__( 'Navbar Color Hover', 'ticketbox' ), 
								'navbar-dd-bg' => esc_html__( 'Navbar Dropdown Background', 'ticketbox' ), 
								'navbar-dd-hover-bg' => esc_html__( 'Navbar Dropdown Hover Background', 'ticketbox' ), 
								'navbar-dd-font' => esc_html__( 'Navbar Dropdown Color', 'ticketbox' ), 
								'navbar-dd-font-hover' => esc_html__( 'Navbar Dropdown Color Hover', 'ticketbox' ), 
								'navbar-dd-mm-title' => esc_html__( 'Navbar Dropdown Mega Menu Title', 'ticketbox' ) ) ) ) ), 
				'footer' => array( 
					'icon' => 'fa fa-list-alt', 
					'title' => esc_html__( 'Footer', 'ticketbox' ), 
					'desc' => __( '<p class="description">Customize Footer.</p>', 'ticketbox' ), 
					'fields' => array( 
						array( 
							'name' => 'footer-area', 
							'type' => 'switch', 
							'on' => esc_html__( 'Show', 'ticketbox' ), 
							'off' => esc_html__( 'Hide', 'ticketbox' ), 
							'label' => esc_html__( 'Footer Widget Area', 'ticketbox' ), 
							'desc' => esc_html__( 
						...
						array( 
							...
							'type' => 'image_select', 
							'label' => esc_html__( 'Footer Area Columns', 'ticketbox' ), 
							'desc' => esc_html__( 
						...
						array( 
							...
							'on' => esc_html__( 'Show', 'ticketbox' ), 
							'off' => esc_html__( 'Hide', 'ticketbox' ), 
							'label' => esc_html__( 'Footer Featured', 'ticketbox' ), 
							'desc' => esc_html__( 'Do you want show featured in footer.', 'ticketbox' ), 
							'value' => '1' ),  // 1 = checked | 0 = unchecked
						array( 
							'name' => 'footer-featured-1', 
							'type' => 'textarea', 
							'dependency' => array( 'element' => 'footer-featured', 'value' => array( '1' ) ), 
							'label' => esc_html__( 'Footer Featured Column 1', 'ticketbox' ), 
							'desc' => esc_html__( 'Footer featured column 1 text.', 'ticketbox' ), 
						...
						array( 
							...
							'type' => 'textarea', 
							'dependency' => array( 'element' => 'footer-featured', 'value' => array( '1' ) ), 
							'label' => esc_html__( 'Footer Featured Column 2', 'ticketbox' ), 
							'desc' => esc_html__( 'Footer featured column 2 text.', 'ticketbox' ), 
						...
						array( 
							...
							'type' => 'textarea', 
							'dependency' => array( 'element' => 'footer-featured', 'value' => array( '1' ) ), 
							'label' => esc_html__( 'Footer Featured Column 3', 'ticketbox' ), 
							'desc' => esc_html__( 'Footer featured column 3 text.', 'ticketbox' ), 
						...
						array( 
							...
							'type' => 'image', 
							'value' => get_template_directory_uri() . '/assets/images/logo.png', 
							'label' => esc_html__( 'Footer Logo', 'ticketbox' ), 
							'desc' => esc_html__( 'Upload your footer logo.', 'ticketbox' ) ), 
						array( 
							'name' => 'footer-info', 
							'type' => 'textarea', 
							'on' => esc_html__( 'Show', 'ticketbox' ), 
							'off' => esc_html__( 'Hide', 'ticketbox' ), 
							'label' => esc_html__( 'Footer Info', 'ticketbox' ), 
						...
						array( 
							...
							'on' => esc_html__( 'Show', 'ticketbox' ), 
							'off' => esc_html__( 'Hide', 'ticketbox' ), 
							'label' => esc_html__( 'Footer Menu', 'ticketbox' ), 
							'desc' => esc_html__( 'Do you want use menu in main footer.', 'ticketbox' ), 
							'value' => '1' ),  // 1 = checked | 0 = unchecked
						array( 
				...
				'blog' => array( 
					...
					'desc' => __( '<p class="description">Customize Blog.</p>', 'ticketbox' ), 
					'fields' => array( 
						array( 
						...
						array( 
							...
							'options' => array( 
								'default' => esc_html__( 'Default', 'ticketbox' ), 
								'medium' => esc_html__( 'Medium', 'ticketbox' ), 
								'grid' => esc_html__( 'Grid', 'ticketbox' ), 
								'masonry' => esc_html__( 'Masonry', 'ticketbox' ), 
								'timeline' => esc_html__( 'Timeline', 'ticketbox' ) ), 
							'value' => 'default' ), 
						array( 
							'name' => 'blogs-columns', 
							'type' => 'image_select', 
						...
						array( 
							...
							'options' => array( 
								'page_num' => esc_html__( 'Page Number', 'ticketbox' ), 
								'loadmore' => esc_html__( 'Load More Button', 'ticketbox' ), 
						...
						array( 
							...
							'on' => esc_html__( 'Show', 'ticketbox' ), 
							'off' => esc_html__( 'Hide', 'ticketbox' ), 
							'label' => esc_html__( 'Show/Hide Title', 'ticketbox' ), 
							'desc' => esc_html__( 
								'In Archive Blog. Show/Hide the post title below the featured', 
								'ticketbox' ), 
							'value' => '1' ),  // 1 = checked | 0 = unchecked
						array( 
							'name' => 'blogs-link-post-title', 
							'type' => 'switch', 
							'label' => esc_html__( 'Link Title To Post', 'ticketbox' ), 
							'desc' => esc_html__( 
								'In Archive Blog. Choose if the title should be a link to the single post page.', 
								'ticketbox' ), 
						...
						array( 
							...
							'on' => esc_html__( 'Show', 'ticketbox' ), 
							'off' => esc_html__( 'Hide', 'ticketbox' ), 
							'label' => esc_html__( 'Show Featured Image', 'ticketbox' ), 
							'desc' => esc_html__( 'In Archive Blog. Show/Hide the post featured Image', 'ticketbox' ), 
							'value' => '1' ),  // 1 = checked | 0 = unchecked
						array( 
						...
						array( 
							...
							'off' => esc_html__( 'Hide', 'ticketbox' ), 
							'label' => esc_html__( 'Comment Meta', 'ticketbox' ), 
							'desc' => esc_html__( 'In Single Blog. Show/Hide the comment meta', 'ticketbox' ), 
							'value' => '1' ),  // 1 = checked | 0 = unchecked
						array( 
						...
						array( 
							...
							'on' => esc_html__( 'Show', 'ticketbox' ), 
							'off' => esc_html__( 'Hide', 'ticketbox' ), 
							'label' => esc_html__( 'Show Next/Prev Post Link On Single Post Page', 'ticketbox' ), 
							'desc' => esc_html__( 
			...
			if ( defined( 'WOOCOMMERCE_VERSION' ) ) {
				$section['woocommerce'] = array( 
					...
					'desc' => __( '<p class="description">Customize Woocommerce.</p>', 'ticketbox' ), 
					'fields' => array( 
						array( 
							'name' => 'woo-cart-nav', 
							'type' => 'switch', 
		...
		public function enqueue_scripts() {
			...
			wp_register_script( 
				...
				DTINC_VERSION, 
				true );
			$dtthemeoptionsL10n = array( 'reset_msg' => esc_js( 'You want reset all options ?', 'ticketbox' ) );
		...
		public function admin_menu() {
			$option_page = add_theme_page( 
				esc_html__( 'Theme Options', 'ticketbox' ), 
				esc_html__( 'Theme Options', 'ticketbox' ), 
				'edit_theme_options', 
				'theme-options', 
		...
		public function admin_bar_render() {
			...
			$wp_admin_bar->add_menu( 
				array( 'parent' => 'site-name', // use 'false' for a root menu, or pass the ID of the parent menu
						...
						'title' => esc_html__( 'Theme Options', 'ticketbox' ), // link title

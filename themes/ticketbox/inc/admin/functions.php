		...
		if( isset($field['callback']) && !empty($field['callback']) ) {
			...
			switch ($field['type']){
				...
				case 'widgetised_sidebars':
					...
					echo '<option value=""' . $selected . '>' . __('Select a sidebar...','ticketbox') . '</option>';
				...
				case 'gallery':
						...
						jQuery(document).ready(function($) {
							$('.dt-meta-gallery-select').on('click',function(e){
								...
								dt_meta_gallery_frame = wp.media({
									title: '<?php echo __('Add Images to Gallery','ticketbox')?>',
									button: {
										text: '<?php echo __('Add to Gallery','ticketbox')?>',
									},
									library: { type: 'image' },
									multiple: true
								});
											...
											dt_meta_gallery_list.append('\
												<li data-id="' + attachment.id +'">\
													<div class="thumbnail">\
														...
														<a href="#" title="<?php echo __('Delete','ticketbox')?>"><?php echo __('x','ticketbox')?></a></li>\
													</div>\
												</li>'
											);
										}
							...
							?>
								<li data-id="<?php echo esc_attr( $attachment_id ) ?>">
									<div class="thumbnail">
										...
										<a title="<?php echo __('Delete','ticketbox') ?>" href="#"><?php echo __('x','ticketbox') ?></a>
									</div>						
								</li>
							<?php
								endif;
					...
					echo '<input type="button" class="button button-primary dt-meta-gallery-select" name="' . $field['id'] . '_button_upload" id="' . $field['id'] . '_upload" value="' . __('Add Gallery Images','ticketbox') . '" /> ';
					if ( ! empty( $field['description'] ) ) {
						echo '<span class="description">' . dt_echo( $field['description'] ) . '</span>';
					}
					echo '</div>';
				...
				case 'media':
					...
					$btn_text = !empty(  $field['value'] ) ? __( 'Change Media', 'ticketbox' ) : __( 'Select Media', 'ticketbox' );
					echo '<div  class="dt-meta-box-field ' . esc_attr( $field['id'] ) . '_field">';
					echo '<label for="' . esc_attr( $field['id'] ) . '">' . esc_html( $field['label'] ) . '</label>';
					...
					echo '<input type="button" class="button" name="' . $field['id'] . '_button_clear" id="' . $field['id'] . '_clear" value="' . __( 'Clear', 'ticketbox' ) . '" />';				
					if ( ! empty( $field['description'] ) ) {
						echo '<span class="description">' . dt_echo( $field['description'] ) . '</span>';
					}
					echo '</div>';
						...
						jQuery(document).ready(function($) {
							...
							$('#<?php echo esc_attr($field['id']) ?>_upload').on('click', function(event) {
								...
								var dt_<?php echo esc_attr($field['id']); ?>_media_frame = wp.media.frames.wp_media_frame = wp.media({
									title: "<?php echo __( 'Select or Upload your Media', 'ticketbox' ); ?>",
									button: {
										text: "<?php echo __( 'Select', 'ticketbox' ); ?>"
									},
								...
								dt_<?php echo esc_attr($field['id']); ?>_media_frame.on('select', function(){
									...
									$this.attr('value', '<?php echo __( 'Change Media', 'ticketbox' ); ?>');
							...
							$('#<?php echo esc_attr($field['id']) ?>_clear').on('click', function(event) {
								var $this = $(this);
								$this.hide();
								$('#<?php echo esc_attr($field['id']) ?>_upload').attr('value', '<?php echo __( 'Select Media', 'ticketbox' ); ?>');
				...
				case 'image':
					...
					$btn_text = !empty( $image_id ) ? __( 'Change Image', 'ticketbox' ) : __( 'Select Image', 'ticketbox' );
					echo '<div  class="dt-meta-box-field ' . esc_attr( $field['id'] ) . '_field">';
					echo '<label for="' . esc_attr( $field['id'] ) . '">' . esc_html( $field['label'] ) . '</label>';
					echo '<div class="dt-meta-image-thumb">' . $output . '</div>';
					echo '<input type="hidden" name="' . $field['name'] . '" id="' . $field['id'] . '" value="' . $field['value'] . '" />';
					echo '<input type="button" class="button button-primary" name="' . $field['id'] . '_button_upload" id="' . $field['id'] . '_upload" value="' . $btn_text . '" /> ';
					echo '<input type="button" class="button" name="' . $field['id'] . '_button_clear" id="' . $field['id'] . '_clear" value="' . __( 'Clear Image', 'ticketbox' ) . '" />';
					if ( ! empty( $field['description'] ) ) {
						echo '<span class="description">' . dt_echo( $field['description'] ) . '</span>';
					}
					?>
						...
						jQuery(document).ready(function($) {
							...
							$('#<?php echo esc_attr($field['id']) ?>_upload').on('click', function(event) {
								...
								var dt_<?php echo esc_attr($field['id']); ?>_image_frame = wp.media.frames.wp_media_frame = wp.media({
									title: "<?php echo __( 'Select or Upload your Image', 'ticketbox' ); ?>",
									button: {
										text: "<?php echo __( 'Select', 'ticketbox' ); ?>"
									},
									library: { type: 'image' },
								...
								dt_<?php echo esc_attr($field['id']); ?>_image_frame.on('select', function(){
									...
									$this.attr('value', '<?php echo __( 'Change Image', 'ticketbox' ); ?>');
									$('#<?php echo esc_attr($field['id']); ?>_clear').css('display', 'inline-block');
								});
							...
							$('#<?php echo esc_attr($field['id']); ?>_clear').on('click', function(event) {
								var $this = $(this);
								$this.hide();
								$('#<?php echo esc_attr($field['id']); ?>_upload').attr('value', '<?php echo __( 'Select Image', 'ticketbox' ); ?>');

...
class DTImportDemo{
	...
	public function admin_menu(){
		add_submenu_page( 'themes.php' , esc_html__('Import Demo','ticketbox') , esc_html__('Import Demo','ticketbox') , 'manage_options' , 'import-demo' , array(&$this,'output') );
	}
	...
	public function output(){
		...
		<div class="dt-message content" style="display:none;">
			...
			<h1 class="dt-message-title"><?php esc_html_e('Importing Demo Content...','ticketbox')?></h1>
			<p class="dt-message-text"><?php _e('Please be patient and do not navigate away from this page while the import is in&nbsp;progress. This can take a while if your server is slow (inexpensive hosting).<br>You will be notified via this page when the import is completed.','ticketbox')?></p>
		</div>
		<div class="dt-message error" style="display:none;">
			<h1 class="dt-message-title"><?php esc_html_e('Error has occured','ticketbox')?></h1>
			<p class="dt-message-text"></p>
		</div>
		<div class="dt-message success" style="display:none;">
			<h1 class="dt-message-title"><?php esc_html_e('Import completed successfully!','ticketbox')?></h1>
			<p class="dt-message-text"><?php _e('Now you can see the result at','ticketbox')?> <a href="<?php echo site_url(); ?>" target="_blank"><?php _e('your site','ticketbox')?></a><br><?php _e('or start customize via','ticketbox')?> <a href="<?php echo admin_url('themes.php?page=theme-options'); ?>"><?php _e('Theme Options','ticketbox')?></a></p>
		</div>

		<form class="dt-importer" action="?page=import-demo" method="post">
			<h1 class="dt-importer-title"><?php _e('Import Demo Data','ticketbox')?></h1>
			...
			<div class="dt-importer-options">
				<div class="dt-importer-option demo-data" style="display: none">
					<label class="dt-importer-option-check">
						...
						<span class="dt-importer-title"><?php _e('Import Demo Data','ticketbox')?></span>
					</label>
				</div>
				<div class="dt-importer-note">
					<strong><?php esc_html_e('Important Notes:','ticketbox')?></strong>
					<ol>
						<li><?php _e('We recommend to run Demo Import on a clean WordPress installation.','ticketbox')?></li>
						<li><?php echo sprintf(__('To reset your installation we recommend %s plugin.','ticketbox'),'<a href="http://wordpress.org/plugins/wordpress-database-reset/" target="_blank">Wordpress Database Reset</a>')?></li>
						<li><?php _e('The Demo Import will not import the images we have used in our live demo, due to copyright/license reasons.','ticketbox')?></li>
						<li><?php _e('Do not run the Demo Import multiple times one after another, it will result in double content.','ticketbox')?></li>
					</ol>
				</div>
				<input type="hidden" name="action" value="import-demo">
				<input class="button-primary" id="run_import_demo_data" type="submit" value="<?php _e('Import','ticketbox')?>">
			</div>
		</form>
		<script>

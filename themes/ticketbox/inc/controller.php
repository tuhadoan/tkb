	...
	class DTController {
		...
		public function facebook_init(){
			...
			if( !isset( $_REQUEST['fb_response'] ) || !isset( $_REQUEST['fb_user'] )) {
				die(json_encode(array('error' => __('Authenication required.', 'ticketbox'))));
			}
			$redirect = '';
			...
			if( !$fb_userid ) {
				die(json_encode(array('error' => __('Please connect your facebook account.', 'ticketbox'))));
			}
			...
			if( !$user_ID ){
				...
				if( !$user_ID )
				{
					if ( !get_option( 'users_can_register' )) {
						die( json_encode( array( 'error' => __('Registration is not open at this time. Please come back later.', 'ticketbox') )));
					}
					if (dt_get_theme_option('facebook_login',0) == 0) {
						die( json_encode( array( 'error' => __('Registration using Facebook is not currently allowed. Please use our Register page', 'ticketbox') )));
					}
					extract( $fb_user );
					$display_name = $name;
					...
					if( empty( $verified ) || !$verified ) {
						die(json_encode(array('error' => esc_html__('Your facebook account is not verified. You have to verify your account before proceed login or registering on this site.', 'ticketbox'))));
					}
			
					$user_email = $email;
					if ( empty( $user_email )) {
						die(json_encode(array('error' => esc_html__('Please click again to login with Facebook and allow the application to use your email address', 'ticketbox'))));
					}
			
					if( empty( $name )) {
						die(json_encode(array('error' => 'empty_name', esc_html__('We didn\'t find your name. Please complete your facebook account before proceeding.', 'ticketbox'))));
					}
			...
			die( json_encode( array( 'loggedin' => true, 'type' => $logintype, 'url' => $redirect, 'siteUrl' => home_url(), 'message' => esc_html__( 'Login successful, redirecting...','ticketbox' ) )));
		...
		public function login(){
			...
			if ( is_wp_error( $user_signon ) ){
				...
				wp_send_json(array('loggedin'=>true, 'redirecturl' => $redirecturl, 'message'=> '<span class="success-response"><i class="fa fa-check-circle"></i> ' . esc_html__( 'Login successful, redirecting...','ticketbox' ) . '</span>' ));
			}
			die();
		}
		public function register(){
			if( !check_ajax_referer('dt-ajax-register-nonce', 'security', false) ) {
				$result = array(
					'success' => false,
					'message' => '<span class="error-response">'.esc_html__( 'Your session is expired or you submitted an invalid form.', 'ticketbox' ).'</span>',
				);
				wp_send_json($result);
			}
			if(get_option( 'users_can_register' )){
				...
				if ( is_wp_error( $errors ) ) {
					...
					$result = array(
						'success'     => true,
						'message'	=> '<span class="success-response">'.esc_html__( 'Registration complete.', 'ticketbox' ).'</span>',
				...
				$result = array(
					'success' => false,
					'message'   =>esc_html__( 'Not allow register in site.', 'ticketbox' ),
				);
			}
			wp_send_json($result);
		}
		protected function _register_new_user($user_login, $user_email, $user_password='', $cuser_password=''){
			...
			if ( $sanitized_user_login == '' ) {
				$errors->add( 'empty_username', esc_html__( 'Please enter a username.', 'ticketbox' ) );
			} elseif ( ! validate_username( $user_login ) ) {
				$errors->add( 'invalid_username', esc_html__( 'This username is invalid because it uses illegal characters. Please enter a valid username.', 'ticketbox' ) );
				$sanitized_user_login = '';
			} elseif ( username_exists( $sanitized_user_login ) ) {
				$errors->add( 'username_exists', esc_html__( 'This username is already registered. Please choose another one.', 'ticketbox' ) );
			}
			
			// Check the email address
			if ( $user_email == '' ) {
				$errors->add( 'empty_email', esc_html__( 'Please type your email address.', 'ticketbox' ) );
			} elseif ( ! is_email( $user_email ) ) {
				$errors->add( 'invalid_email', esc_html__( 'The email address isn\'t correct.', 'ticketbox' ) );
				$user_email = '';
			} elseif ( email_exists( $user_email ) ) {
				$errors->add( 'email_exists', esc_html__( 'This email is already registered, please choose another one.', 'ticketbox' ) );
			}
			//Check the password
			...
			if(empty($user_password)){
				...
				if(strlen($user_password) < 6){
					$errors->add( 'minlength_password', esc_html__( 'Password must be 6 character long.', 'ticketbox' ) );
				}elseif (empty($cuser_password)){
					$errors->add( 'not_cpassword', esc_html__( 'Not see password confirmation field.', 'ticketbox' ) );
				}elseif ($user_password != $cuser_password){
					$errors->add( 'unequal_password', esc_html__( 'Passwords do not match.', 'ticketbox' ) );
				}
			}
			...
			if ( ! $user_id ) {
				$errors->add( 'registerfail', esc_html__( 'Couldn\'t register you... please contact the site administrator', 'ticketbox' ) );
				return $errors;
			}
			...
			// user email
			$subject = sprintf(__('Welcome to [%1$s]','ticketbox'),$blogname);
			$to = $user->email;
...
%4$s','ticketbox');
		...
		public function register_priv(){
			...
			wp_send_json(array('success'=>true, 'message'=> sprintf(__('You are already logged in. Please <a href="#" onclick="%s">refresh</a> page','ticketbox'),$link)));
			die();
		}
		
		public function login_priv(){
			$link = "javascript:window.location.reload();return false;";
			wp_send_json(array('loggedin'=>true, 'message'=> sprintf(__('You are already logged in. Please <a href="#" onclick="%s">refresh</a> page','ticketbox'),$link)));
			die();
		}
		...
		public function lostpassword(){
			...
			if ( isset($_POST) ) {
				...
				if ( empty( $_POST['user_login'] ) ) {
					$errors->add('empty_username', __('<strong>ERROR</strong>: Enter a username or e-mail address.','ticketbox'));
				} else if ( strpos( $_POST['user_login'], '@' ) ) {
					$user_data = get_user_by( 'email', trim( $_POST['user_login'] ) );
					if ( empty( $user_data ) )
						$errors->add('invalid_email', __('<strong>ERROR</strong>: There is no user registered with that email address.','ticketbox'));
				} else {
				...
				if ( !$user_data ) {
					$errors->add('invalidcombo', __('<strong>ERROR</strong>: Invalid username or e-mail.','ticketbox'));
					echo '<span class="error-response">' . $errors->get_error_message() . '</span>';
					die();
				}
				...
				*/
				...
				if ( ! $allow ) {
					echo '<span class="error-response">' . esc_html__('Password reset is not allowed for this user','ticketbox') . '</span>';
					die();
				}
				...
				*/
				...
				$message = esc_html__('Someone requested that the password be reset for the following account:','ticketbox') . "\r\n\r\n";
				$message .= network_home_url( '/' ) . "\r\n\r\n";
				$message .= sprintf(__('Username: %s','ticketbox'), $user_login) . "\r\n\r\n";
				$message .= esc_html__('If this was a mistake, just ignore this email and nothing will happen.','ticketbox') . "\r\n\r\n";
				$message .= esc_html__('To reset your password, visit the following address:','ticketbox') . "\r\n\r\n";
				...
				*/
				...
				$title = sprintf( __('[%s] Password Reset','ticketbox'), $blogname );
				...
				*/
				...
				if ( $message && !wp_mail( $user_email, wp_specialchars_decode( $title ), $message ) ) {
					echo '<span class="error-response">' . __("Failure!", 'ticketbox');
					echo esc_html__('The e-mail could not be sent.','ticketbox');
					echo "</span>";
					die();
				} else {
					echo '<span class="success-response">' . esc_html__("Email successfully sent!", 'ticketbox')."</span>";
					die();
				}
			}
			die();
		...
		protected function _mailchimp_subscribe() {
			...
			if(!wp_verify_nonce( $nonce, 'mailchimp_subscribe_nonce' )){
				wp_die(esc_html__('Verify Nonce Failed','ticketbox'));
			}
			if(!filter_var(@$data['email'], FILTER_VALIDATE_EMAIL)){
				wp_die(esc_html__('Invalid Email','ticketbox'));
			}
			if ( $mailchimp_api ) {
		...
		public function search() {
			...
			if ( empty( $posts ) ) {
				...
				$output .= esc_html__( 'No results matched!', 'ticketbox' );
				$output .= '</div>';
			} else {
				$post_types = array();
				...
				$output .= '<div class="search-view-all"><a href="' . home_url( '?s=' . $_REQUEST['s'] ) . '">' .
					 esc_html__( 'View all results', 'ticketbox' ) . '</a>';
			}

 ...
 */
function dt_excerpt( $class = 'entry-summary' ) {
...
function dt_social($use = array(),$hover = true,$soild_bg=false,$outlined=false){
	...
	foreach ((array)$socials  as $social=>$data):
		if(in_array($social, $use)):
			if(empty($data['url']))
				$data['url'] = '#';
			echo '<a href="'.esc_url($data['url']).'" title="'.esc_attr($data['label']).'" ><i class="fa fa-'.$social.' '.($hover ? $social.'-bg-hover':'').' '.($soild_bg ? $social.'-bg':'').' '.($outlined ? $social.'-outlined':'').'"></i></a>';
		endif;
	endforeach;
	return ;
}
...
function dt_list_comments($comment, $args, $depth) {
	global $post;
